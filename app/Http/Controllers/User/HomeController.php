...
class HomeController extends Controller
{
    ...
    public function form_post(Request $request)
    {
        ...
        try {
            ...
            // send email
            mail("Info@adib.web.tr", "فرم درخواست پروژه", $msg);
    ...
    public function index($lang = null)
    {
        ...
        $services = Service::where('lang' ,  session()->get('locale'))->orderBy('sort' , 'ASC')->get();
        $this->locale = session()->get('locale');
        ...
        return view('index', compact('setting', 'customers', 'customer_icon', 'galleries', 'sliders', 'certificate', 'news', 'article', 'proShow', 'news1','services'));
    }
...
//        dd(Article::where('lang','tr')->where('slug','seo-nuzu-neden-bize-bırakmalısınız?')->get());
    ...
    public function template()
    {
        ...
        $customers = Customer::where('lang', app()->getLocale())->orderBy('sort', 'asc')->paginate(12);
        $categories = CustomerCategory::where('lang', $this->locale)->orderBy('sort_id', 'ASC')->get();


        return view('gallery', compact('customers', 'categories'));
    }
...
//    }
    public function templateCat($lang, $slug)
    {
...
//        {
        ...
        $customers = $customersCollection->sortBy('sort')->paginate(10);
//        }
        $slugs = $cat->slug;
        return view('gallery', compact('customers', 'categories', 'cat', 'slugs'));
    ...
    public function templateType($lang, $slug)
    {
        ...
        $customers = $customers->filter(function ($customer) use ($type) {
            if (in_array($type->id,json_decode($customer->type_id))) {
                return $customer;
            }
        });
